        ...
        {isBooked && bookedData ? (
          ...
          >
            <div className="bg-white/5 border border-white/10 rounded-xl p-5 flex flex-col gap-3">
              ...
              {bookedData.method?.includes("Google Meet") && (() => {
                const startDate = bookedData.preferred_date.replace(/-/g, "");
                const [h, m] = (bookedData.preferred_time || "10:00").split(":");
                const startTime = `${startDate}T${h}${m}00`;
                const endH = String(Number(h) + 1).padStart(2, "0");
                const endTime = `${startDate}T${endH}${m}00`;
                const calUrl = `https://calendar.google.com/calendar/render?action=TEMPLATE&text=${encodeURIComponent("Studio HaeTae 1차 미팅")}&dates=${startTime}/${endTime}&details=${encodeURIComponent("Studio HaeTae 프로젝트 미팅")}&add=meet`;
                return (
                  <a
                    href={calUrl}
                    target="_blank"
