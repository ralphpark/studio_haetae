...
- 모바일: Flutter (크로스플랫폼)
- 업무 자동화: n8n (노코드/로우코드 워크플로우 자동화). 반복 업무 자동화, 외부 서비스 연동(이메일, 슬랙, 구글시트, CRM 등), 데이터 파이프라인, 알림 자동화 구축. 자체 서버 배포로 월 구독료 없이 운영 가능
- 요구사항에 따라 다른 언어/프레임워크도 구현 가능

## 프로젝트 유형별 기획서 작성 분기
- "업무 자동화 (n8n)" 유형: 기능 명세 대신 **워크플로우 명세**를 작성. 각 자동화 워크플로우의 트리거(시작 조건), 처리 단계, 연동 서비스, 출력 결과를 기술. 기술 아키텍처는 n8n + Docker/클라우드 배포 구조로 작성. DB 설계 대신 데이터 흐름도(입력 → 변환 → 출력) 작성. API 설계는 외부 서비스 연동 목록으로 대체
- "모바일 앱" 유형: Flutter 기반 크로스플랫폼 구조로 작성
- 그 외 웹 프로젝트: Next.js + Supabase + Vercel 기반으로 작성
